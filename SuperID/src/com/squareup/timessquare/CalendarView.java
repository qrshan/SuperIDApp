...
public class CalendarView extends LinearLayout {
	...
	public static CalendarView create(ViewGroup parent,
			LayoutInflater inflater, DateFormat weekdayNameFormat,
			CellClickListener listener, Calendar today) {
		final CalendarView view = (CalendarView) inflater.inflate(
	...
	/*
	 * public static MonthView create(Context context, ViewGroup parent) { final
	 * MonthView view = (MonthView) LayoutInflater.from(context)
	 ...
	 * @param dates
	 */
	public void markDatesOfMonth(int year, int month, boolean markIcon,
			boolean markText, Integer... dates) {
	...
	public void markDatesOfMonth(int year, int month, boolean markIcon,
			boolean markText, List<Calendar> calendars) {
		Calendar cal = Calendar.getInstance(locale);
	...
	public List<List<MonthCellDescriptor>> getMonthCells(MonthDescriptor month,
		...
		/*
		 * Calendar minSelectedCal = minDate(selectedCals); Calendar
		 * maxSelectedCal = maxDate(selectedCals);
		 */
		...
		while ((cal.get(MONTH) < month.getMonth() + 1 || cal.get(YEAR) < month
			...
			for (int c = 0; c < 7; c++) {
				...
				/*
				 * if (selectedCals != null && selectedCals.size() > 1) { if
				 * (sameDate(minSelectedCal, cal)) { rangeState =
				 * MonthCellDescriptor.RangeState.FIRST; } else if
				 * (sameDate(maxDate(selectedCals), cal)) { rangeState =
				 * MonthCellDescriptor.RangeState.LAST; } else if
	...
	private boolean markTextCalsContainsDate(Calendar cal) {
		List<Calendar> selectedCals = markTextCalsMap.get(monthNameFormat
				.format(cal.getTime()));
		return (selectedCals != null) ? containsDate(selectedCals, cal) : false;
	}

	private boolean markCalsContainsDate(Calendar cal) {
		List<Calendar> selectedCals = markCalsMap.get(monthNameFormat
				.format(cal.getTime()));
		return (selectedCals != null) ? containsDate(selectedCals, cal) : false;
	}
	 ...
	 * { this.cellClickListener = cellClickListener; for (int i = 0; i < 6; i++)
	 * { CalendarRowView weekRow = (CalendarRowView) grid.getChildAt(i + 1);
	 * weekRow.setCellClickListener(cellClickListener); } }
	 */
